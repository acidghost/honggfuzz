...
static bool fuzz_injectFiles(run_t * run)
{
    struct dirent **namelist;
    int nfiles = scandir(run->global->io.injectDir, &namelist, fuzz_filterInject, alphasort);

    if (nfiles < 0) {
        PLOG_W("scandir('%s', &namelist, fuzz_filterInject, alphasort)",
            run->global->io.injectDir);
        return false;
    }

    ssize_t maxInjectLast = run->global->io.injectLast;
    for (size_t i = 0; i < (size_t) nfiles; i++) {
        ...
        if (run->global->io.injectLast >= 0 && file_id <= (size_t) run->global->io.injectLast) {
            free(file_d);
            continue;
        }

        char fname[PATH_MAX];
        snprintf(fname, PATH_MAX - 1, "%s/%s", run->global->io.injectDir, file_d->d_name);
        free(file_d);
        ...
        {
            MX_SCOPED_RWLOCK_WRITE(&run->global->dynfileq_mutex);
            TAILQ_INSERT_HEAD(&run->global->dynfileq, dynfile, pointers);
            run->global->dynfileqCnt++;
            run->stats->injectedInputs++;
        }
    ...
    free(namelist);
    ATOMIC_SET(run->global->io.injectLast, maxInjectLast);

    return true;
}

static bool fuzz_logStats(run_t * run)
{
    /* Re-open file so we can overwrite it */
    run->statsFile = freopen(NULL, "w", run->statsFile);
    if (run->statsFile == NULL) {
        PLOG_W("failed to reopen %s", run->statsFileName);
        return false;
    }

    fuzzStats_t *stats = run->stats;
    fprintf(run->statsFile, "executedInputs: %zu\ninjectedInputs: %zu\n",
...
static void* fuzz_threadNew(void *arg)
{
    ...
    bool useStatsFile = hfuzz->io.statsDir != NULL;
    if (useStatsFile) {
        snprintf(statsFileName, PATH_MAX - 1, "%s/stats.%d",
            hfuzz->io.statsDir, fuzzNo);
    ...
    for (;;) {
        ...
        if (hfuzz->io.injectDir != NULL &&
            (ATOMIC_GET(hfuzz->cnts.mutationsCnt) % syncInjectedTh) == 0)
        {
            if (!fuzz_injectFiles(&run)) {
        ...
        /* Log stats to file */
        if (run.statsFile != NULL &&
            (ATOMIC_GET(hfuzz->cnts.mutationsCnt) % _HF_LOG_STATS_EVERY) == 0)
        {
            if (!fuzz_logStats(&run)) {
